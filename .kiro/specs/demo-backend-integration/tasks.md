...
- [ ] 1. Extract shared device-state source (unblocks backend, avoids circular import)
  - [-] 1.1 Create `alexa-thinks-ahead/src/devices/demo_states.py`
  ...
  - [~] 1.2 Point `demo.py` `SimulatedAdapter` at the shared module
...
- [ ] 2. Backend HTTP layer in `demo.py` `DemoHandler`
  - [~] 2.1 Add CORS preflight + `/api/v1` prefix strip + path-param dispatch
  ...
  - [~] 2.2 Example test: OPTIONS preflight and CORS headers
...
- [ ] 3. Backend response bodies in `src/handlers/api_handler.py`
  - [~] 3.1 Device list + single device state handlers
  ...
  - [~] 3.3 Patterns, tiers, and tier-update handlers
  ...
  - [~] 3.4 Property tests: routing parity + body schema coverage
...
- [ ] 4. Backend live power-cut scenario
  - [~] 4.1 Add `POST /scenario/power-cut` route and handler
  ...
  - [~] 4.2 Scenario wiring tests (example + property)
...
- [ ] 5. Frontend data layer
  - [-] 5.1 Confirm `DataLayer` default mock + manual toggle
  ...
  - [~] 5.2 `ApiProvider` normalization + `runPowerCutScenario`
  ...
  - [~] 5.3 Frontend property tests: mode routing + reshape
...
- [ ] 6. Frontend live scenario wiring
  - [~] 6.1 Wire power-cut control to POST + animate from real action plan
  ...
  - [~] 6.2 Error indication + scripted fallback on failure
  ...
  - [~] 6.3 Frontend scenario tests (example + property)
...
- [ ] 7. Checkpoint and end-to-end validation
  - [~] 7.1 Checkpoint — run backend and frontend test suites
    - Ensure backend (`pytest`) and frontend (`npm run test`) suites pass. Ensure all tests pass, ask the user if questions arise.

  - [~] 7.2 Manual end-to-end smoke test (presenter, manual)
